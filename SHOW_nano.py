...
# cap_front.set(cv2.CAP_PROP_EXPOSURE, -7)  # 曝光 -4
ratio = cap.get(cv2.CAP_PROP_FRAME_WIDTH)/cap.get(cv2.CAP_PROP_FRAME_HEIGHT)
WIDTH = 700
HEIGHT = int(WIDTH/ratio)

lower_red = np.array([0,193,0])  
...
upper_black = np.array([226,53,57]) 

lower_red_water = np.array([0,87,0])  
upper_red_water = np.array([25,255,255])  
lower_blue_water = np.array([85,48,0])  
upper_blue_water = np.array([146,255,255]) 
lower_yellow_water = np.array([21,93,0])  
upper_yellow_water = np.array([83,225,255]) 
# lower_black = np.array([0,0,0])  
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~^LOAD^~~~~~~~~~~~~~~~
#load_model(): device, model, stride, half
device = load_model()[0]
model = load_model()[1]
stride = load_model()[2]
half = load_model()[3]
#
def detect(fruit_class, device=device, model=model, stride=stride, half=half, save_img=False, source='', weights='', view_img=True, save_txt=False, imgsz=416, trace=True):
...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~^CUT^~~~~~~~~~~~~~~~
    ...
    imgContour = img.copy()
    lower_board = np.array([0, 88, 141])
    upper_board = np.array([148, 255, 255])
...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~^MASK^~~~~~~~~~~~~~~~
    ...
    cv2.circle(imgContour, (300, 170), 5, (2, 255, 75), -1)
    cv2.circle(imgContour, (300, 270), 5, (2, 255, 75), -1)
    cv2.circle(imgContour, (400, 170), 5, (2, 255, 75), -1)
    cv2.circle(imgContour, (400, 270), 5, (2, 255, 75), -1)
    ...
    try:
        if len(contours) > 0:
            for cnt in contours:
                ...
                if cX > (WIDTH/2)-50 and cX < (WIDTH/2)+50 and cY > (HEIGHT/2)-92 and cY < (HEIGHT/2)+8:
                    print("in~!")
                    # cv2.drawContours(imgContour, cnt, -1, (255, 0, 0), 4)
                    # cv2.circle(imgContour, (cX, cY), 5, (213, 255, 75), -1)
                    ...
                    if area > 10000:
                        ...
                        if corners == 3:
                            direction = [cX-vertices[0][0][0], cX -
                                         vertices[1][0][0], cX-vertices[2][0][0]]
                            if sum(i > 0 for i in direction) == 1:
                                # if area > 9000:
                                cv2.putText(
                                    imgContour, 'left triangle', (x, y-5), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 0), 2)
                                LT_num = LT_num+1
                                ...
                                # if area > 9000:
                                cv2.putText(
                                    imgContour, 'right triangle', (x, y-5), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 0), 2)
                                RT_num = RT_num+1
                                return ("no_shape", imgContour)
...
while True:#顯示影片
        ...
        #ret_front, frame_front = cap_front.read()  # 讀取影片每一幀，cap.read()回傳兩個值，第一個為布林值，取得道的下一幀 
        if ret:                     
            try :
                #print("here") 
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~^buttom^~~~~~~~~                
                while(walk_1 == True) :
                    ...
                    break
                while(turn == True):
                    arduino_turn = ser.readline()
                    print(arduino_turn)
                    if(arduino_turn == b'TURN_WATER\n'):
                        water = True
                        turn = False
                        img = cv2.imread("white.png")
                        time.sleep(3)
                    if(arduino_turn == b'TURN_CATCH\n'):
                        water = True
                        turn = False
                        img = cv2.imread("white.png")
                        time.sleep(3)
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~^catch^~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
                while(water == True) :
                    ...
                    try:
                        ...
                        color_img = cv2.imread("white.png")
                        color = "nothing"
                        continue
                    ...
                    while(SEEN == True):
                        while(True):
                            ...
                            try:
                                if (color == "red"):
                                    result_img = WaterColor(frame,upper_red_water,lower_red_water)[2]
                                    water_X=WaterColor(frame,upper_red_water,lower_red_water)[0]
                                if(color == "y"):
                                    result_img = WaterColor(frame,upper_yellow_water,lower_yellow_water)[2]
                                    water_X=WaterColor(frame,upper_yellow_water,lower_yellow_water)[0]
                                if (color == "blue"):
                                    result_img = WaterColor(frame,upper_blue_water,lower_blue_water)[2]
                                    water_X=WaterColor(frame,upper_blue_water,lower_blue_water)[0]
                                # if (color == "black"):
                                ...
                                while(found_x == True):
                                    while(True):
                                        ...
                                        try:
                                            if (color == "red"):
                                                result_img = WaterColor(frame,upper_red_water,lower_red_water)[2]
                                                water_Y=WaterColor(frame,upper_red_water,lower_red_water)[1]
                                            if(color == "y"):
                                                result_img = WaterColor(frame,upper_yellow_water,lower_yellow_water)[2]
                                                water_Y=WaterColor(frame,upper_yellow_water,lower_yellow_water)[1]
                                            if (color == "blue"):
                                                result_img = WaterColor(frame,upper_blue_water,lower_blue_water)[2]
                                                water_Y=WaterColor(frame,upper_blue_water,lower_blue_water)[1]
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~^U_walk^~~~~~~~~                      
                ...
                result_img = cv2.flip(frame,1)
                cv2.imshow("false", result_img)
                print('發生錯誤')
        else:
            break
